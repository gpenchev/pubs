...
source(file.path("modules", "mod_help.R"))
...
# Convenience: conflict events (pre-filtered, 787 rows) — land-contiguous only
conflict_events <- app_data[["app_conflict_events"]]

# All events (no land-contiguity filter) — used for map toggle.
# Generated by 11_app_data.R from the full ucdp_map_events.rds.
# NULL if the file is absent (toggle will be hidden gracefully).
all_events <- app_data[["app_all_events"]]
